...
CREATE TABLE tables(
    ...
    |   |   |   |   CHECK(table_status IN 'Available', 'Occupied', 'Reserved', 'Maintainance')
    is_active       BOOLEAN         DEFAULT TRUE,
    created_at      TIMESTAMP       DEFAULT NOW(),
    updated_at      TIMESTAMP       DEFAULT NOW()
);
CREATE TABLE orders(
  order_id        SERIAL PRIMARY KEY,
    ...
    order_status    VARCHAR(20)     DEFAULT 'pending'
                    CHECK (order_status IN ('pending', 'confirmed', 'preparing', 'ready', 'served', 'cancelled')),
    special_notes   TEXT,
...
CREATE TABLE order_items(
    ...
    menu_item_id    INT             NOT NULL REFERENCES menu_items(menu_item_id) ON DELETE RESTRICT,
    quantity        INT             NOT NULL DEFAULT 1,              
    unit_price      DECIMAL(10,2)   NOT NULL,                        
    subtotal        DECIMAL(10,2)   NOT NULL,
    special_request TEXT,
    order_item_status VARCHAR(20)   DEFAULT 'pending'
                    CHECK (order_item_status IN ('pending', 'preparing', 'ready', 'served', 'cancelled')),
    created_at      TIMESTAMP       DEFAULT NOW(),
    updated_at      TIMESTAMP       DEFAULT NOW()
);
...
CREATE TABLE reservations(
    ...
    reservation_status  VARCHAR(20)     DEFAULT 'pending'
                        CHECK (reservation_status IN ('pending', 'confirmed', 'seated', 'cancelled', 'no_show')),
...
CREATE TABLE inventory(
    ...
    unit            VARCHAR(50)     NOT NULL,                        -- changed INT → VARCHAR; units are 'kg', 'liters', 'pieces'
    quantity        DECIMAL(10,2)   DEFAULT 0,                       -- DECIMAL better than INT for partial units (0.5 kg)
    reorder_level   DECIMAL(10,2)   DEFAULT 0,                       -- was missing data type
    cost_per_unit   DECIMAL(10,2)   DEFAULT 0.00,
    supplier        VARCHAR(300),                                    -- removed NOT NULL; supplier may not always be known
    is_active       BOOLEAN         DEFAULT TRUE,                    -- DEFAULT VALUE YES is invalid
    created_at      TIMESTAMP       DEFAULT NOW(),
    updated_at      TIMESTAMP       DEFAULT NOW()
);
...
CREATE TABLE kitchen_display
    ...
    order_status        VARCHAR(20)     DEFAULT 'received'
                        CHECK (order_status IN ('received', 'started', 'in_progress', 'completed', 'delivered')),
    order_priority      VARCHAR(20)     DEFAULT 'in_queue'
                        CHECK (order_priority IN ('urgent', 'in_queue')),
