...
import { BusinessService } from './businessServic'
...
// Import your actual service methods
// Replace these with your actual service imports
import { ordersService } from '../../app/modules/orders/services/orderService'
import { categoriesService } from '../../app/modules/pos/services/categoriesService'
import { productsService } from '../../app/modules/products/services/productService'
import { tablesService } from '../modules/pos/services/tablesService'
...
export class BackupService {
    ...
    static async createBackup(): Promise<BackupData> {
        try {
            ...
            // Get other data using direct service calls
            const [products, categories, tables, orders, logs, printerConfigs] = await Promise.all([
                ...
                logsService.getLogs().catch(() => []),
                // Printer configs would need a service - placeholder for now
    ...
    static async exportAndShare(options: ExportOptions): Promise<void> {
        try {
            const data = await this.exportData(options)
            const fileName = `backup_${new Date().toISOString().split('T')[0]}.${options.format === 'excel' ? 'csv' : options.format}`
    ...
    // Export to Excel (simplified - returns CSV format)
    ...
    static async saveBackupToFile(backupData: BackupData): Promise<string> {
        try {
            ...
            const fileName = `backup_${backupData.timestamp.split('T')[0]}.json`
            const fileUri = `${FileSystem.documentDirectory}${fileName}`
    ...
    static async loadBackupFromFile(fileUri: string): Promise<BackupData> {
        try {
    ...
    private static async importProducts(products: any[], mode: string): Promise<void> {
        const existingProducts = await productsService.getProducts().catch(() => []) as any[]

        for (const product of products) {
            if (mode === 'replace') {
                // Always create new (current behavior)
                await productsService.createProduct(product).catch(console.error)
            } else if (mode === 'merge') {
    ...
    private static async importCategories(categories: any[], mode: string): Promise<void> {
        const existingCategories = await categoriesService.getCategories().catch(() => []) as any[]

        for (const category of categories) {
            if (mode === 'replace') {
                // Always create new
                await categoriesService.createCategory(category).catch(console.error)
            } else if (mode === 'merge') {
                ...
                if (existing) {
                    ...
                    await categoriesService.updateCategory(existing.id, {
                        name: category.name,
    ...
    private static async importTables(tables: any[], mode: string): Promise<void> {
        const existingTables = await tablesService.getTables().catch(() => []) as any[]

        for (const table of tables) {
            if (mode === 'replace') {
                // Always create new
                await tablesService.createTable(table).catch(console.error)
            } else if (mode === 'merge') {
                // Check for duplicates by name
                ...
                if (existing) {
                    ...
                    await tablesService.updateTable(existing.id, {
                        name: table.name,
    ...
    // Get available backup files from document directory
