...
export class BackupService {
    ...
    static async createBackup(): Promise<BackupData> {
        try {
            ...
            // Get other data using Django API calls
            const [products, categories, tables, orders, logs, printerConfigs] = await Promise.all([
                axios.get(`${API_BASE_URL}/api/products/`).then(res => res.data).catch(() => []),
                axios.get(`${API_BASE_URL}/api/categories/`).then(res => res.data).catch(() => []),
                axios.get(`${API_BASE_URL}/api/tables/`).then(res => res.data).catch(() => []),
                axios.get(`${API_BASE_URL}/api/orders/`).then(res => res.data).catch(() => []),
                axios.get(`${API_BASE_URL}/api/logs/`).then(res => res.data).catch(() => []),
                axios.get(`${API_BASE_URL}/api/printer-configs/`).then(res => res.data).catch(() => null),
            ]);
    ...
    private static exportToExcel(data: any, domains: string[]): string {
        ...
        // In a real implementation, you'd use a library like 'react-native-xlsx' or 'xlsx'
        return this.exportToCSV(data, domains);
    }

    // Save backup to file (for React Native)
    static async saveBackupToFile(backupData: BackupData): Promise<string> {
        try {
            const jsonData = JSON.stringify(backupData, null, 2);

            // In React Native, you'd use react-native-fs to save files
            // Example:
            // import RNFS from 'react-native-fs';
            // const path = `${RNFS.DocumentDirectoryPath}/backup_${Date.now()}.json`;
    ...
    private static async importProducts(products: any[], mode: string): Promise<void> {
        const response = await axios.get(`${API_BASE_URL}/api/products/`).catch(() => ({ data: [] }));
        const existingProducts = response.data;

        for (const product of products) {
            if (mode === 'replace') {
                // Always create new (current behavior)
                await axios.post(`${API_BASE_URL}/api/products/`, product).catch(console.error);
            } else if (mode === 'merge') {
                ...
                if (existing) {
                    ...
                    await axios.patch(`${API_BASE_URL}/api/products/${existing.id}/`, {
                        name: product.name,
    ...
    private static async importCategories(categories: any[], mode: string): Promise<void> {
        const response = await axios.get(`${API_BASE_URL}/api/categories/`).catch(() => ({ data: [] }));
        const existingCategories = response.data;

        for (const category of categories) {
            if (mode === 'replace') {
                // Always create new
                await axios.post(`${API_BASE_URL}/api/categories/`, category).catch(console.error);
            } else if (mode === 'merge') {
                // Check for duplicates by name
                const existing = existingCategories.find((c: any) => c.name === category.name);

                if (existing) {
                    // Update existing category
                    await axios.patch(`${API_BASE_URL}/api/categories/${existing.id}/`, {
                        name: category.name,
    ...
    private static async importTables(tables: any[], mode: string): Promise<void> {
        const response = await axios.get(`${API_BASE_URL}/api/tables/`).catch(() => ({ data: [] }));
        const existingTables = response.data;

        for (const table of tables) {
            if (mode === 'replace') {
                // Always create new
                await axios.post(`${API_BASE_URL}/api/tables/`, table).catch(console.error);
            } else if (mode === 'merge') {
                // Check for duplicates by name
                const existing = existingTables.find((t: any) => t.name === table.name);

                if (existing) {
                    // Update existing table
                    await axios.patch(`${API_BASE_URL}/api/tables/${existing.id}/`, {
                        name: table.name,
    ...
    static async getAvailableBackups(): Promise<string[]> {
        try {
            // In React Native, you'd use react-native-fs to scan backup directory
            // Example:
            // import RNFS from 'react-native-fs';
            // const files = await RNFS.readDir(RNFS.DocumentDirectoryPath);
            // return files.filter(f => f.name.startsWith('backup_')).map(f => f.name);
            
            // Or fetch from Django API if backups are stored server-side
